...
for threshold in [500]:
    for gp in [ 0]:
        for model in ['lgb', 'knn', 'rf', ]:
            for deep in [4]:
                   for  file in [ '3gp237','4gp95' ]: #,,, '4gp95'
                       for n_neighbors in [19]:
                           for p in [5]:
                                ...
                                val_df = process_df(train, test, threshold, gp, model, max_depth=deep, num_round=100,n_neighbors = n_neighbors)
